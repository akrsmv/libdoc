...
export class Company implements IDdbItemBaseProps {
  static isPublicItem = false
  static __typename = "Company"
  static __parent_typename = "Company"
  ...
  static metadataPerKey:Map<keyof IDdbItemBaseProps | keyof Company, DomainPropertyMetadata<Company>> = new Map<keyof IDdbItemBaseProps | keyof Company, DomainPropertyMetadata<Company>>([
    ['date_created',{
      key: 'date_created',
      type: 'isodatestring'
    }],
  ...
  //#endregion
  constructor({ args, identity }: { args?: CompanyDto & IDdbItemKey & { isPublic?: boolean }, identity: Partial<IIdentity<Partial<IClaims>>> | null}) {
...
//         if (Company.metadataPerKey.has(arg as keyof Company)
// 
...
//           // && Company.metadataPerKey.get(arg as keyof Company)?.type === typeof args[arg]
//           ) {
...
//     )
    ...
    if (!!args?.HASH && !!args?.RANGE) {
      pk = existingPKWithoutPrefix(Company.__typename, { HASH: args.HASH, RANGE: args.RANGE })
    } else {
      pk = newPK(Company.__typename, Company.__typename)
    }
    this.__typename = Company.__typename
    this.HASH = withPrefix(pk.HASH, Company.isPublicItem || !!args?.isPublic, identity),
    this.RANGE = withPrefix(pk.RANGE, Company.isPublicItem || !!args?.isPublic, identity),
