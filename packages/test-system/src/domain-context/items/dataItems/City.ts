...
import { CityCityDetailDto } from './dtos/CityCityDetailDto'
...
export class City implements IDdbItemBaseProps {
  static isPublicItem = true
  static __typename = "City"
  static __parent_typename = "City"
  ...
  static metadataPerKey:Map<keyof IDdbItemBaseProps | keyof City, DomainPropertyMetadata<City>> = new Map<keyof IDdbItemBaseProps | keyof City, DomainPropertyMetadata<City>>([
    ['date_created',{
      key: 'date_created',
      type: 'isodatestring'
    }],
    ...
    ['cityDetails', {
      key: 'cityDetails',
      type: 'object', objectTypeDtoName: 'CityCityDetailDto',
  ...
  //#endregion
  constructor({ args, identity }: { args?: CityDto & IDdbItemKey & { isPublic?: boolean }, identity: Partial<IIdentity<Partial<IClaims>>> | null}) {
...
//         if (City.metadataPerKey.has(arg as keyof City)
// 
...
//           // && City.metadataPerKey.get(arg as keyof City)?.type === typeof args[arg]
//           ) {
...
//     )
    ...
    if (!!args?.HASH && !!args?.RANGE) {
      pk = existingPKWithoutPrefix(City.__typename, { HASH: args.HASH, RANGE: args.RANGE })
    } else {
      pk = newPK(City.__typename, City.__typename)
    }
    this.__typename = City.__typename
    this.HASH = withPrefix(pk.HASH, City.isPublicItem || !!args?.isPublic, identity),
    this.RANGE = withPrefix(pk.RANGE, City.isPublicItem || !!args?.isPublic, identity),
  ...
  public cityDetails?: CityCityDetailDto
                    }