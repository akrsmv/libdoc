...
export class Person implements IDdbItemBaseProps {
  static isPublicItem = false
  static __typename = "Person"
  static __parent_typename = "Person"
  ...
  static metadataPerKey:Map<keyof IDdbItemBaseProps | keyof Person, DomainPropertyMetadata<Person>> = new Map<keyof IDdbItemBaseProps | keyof Person, DomainPropertyMetadata<Person>>([
    ['date_created',{
      key: 'date_created',
      type: 'isodatestring'
    }],
  ...
  //#endregion
  constructor({ args, identity }: { args?: PersonDto & IDdbItemKey & { isPublic?: boolean }, identity: Partial<IIdentity<Partial<IClaims>>> | null}) {
...
//         if (Person.metadataPerKey.has(arg as keyof Person)
// 
...
//           // && Person.metadataPerKey.get(arg as keyof Person)?.type === typeof args[arg]
//           ) {
...
//     )
    ...
    if (!!args?.HASH && !!args?.RANGE) {
      pk = existingPKWithoutPrefix(Person.__typename, { HASH: args.HASH, RANGE: args.RANGE })
    } else {
      pk = newPK(Person.__typename, Person.__typename)
    }
    this.__typename = Person.__typename
    this.HASH = withPrefix(pk.HASH, Person.isPublicItem || !!args?.isPublic, identity),
    this.RANGE = withPrefix(pk.RANGE, Person.isPublicItem || !!args?.isPublic, identity),
