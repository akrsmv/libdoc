...
import { queryBeDto } from './dtos/queryBeDto'
import {
  IClaims, IIdentity,
  _sep2,
...
export class queryBe implements IDdbItemBaseProps {
  static isPublicItem = false
  static __typename = "queryBe"
  static __parent_typename = "queryBe"
  ...
  static metadataPerKey:Map<keyof IDdbItemBaseProps | keyof queryBe, DomainPropertyMetadata<queryBe>> = new Map<keyof IDdbItemBaseProps | keyof queryBe, DomainPropertyMetadata<queryBe>>([
    ['date_created',{
      key: 'date_created',
      type: 'isodatestring'
    }],
  ...
  //#endregion
  constructor({ args, identity }: { args?: queryBeDto & IDdbItemKey & { isPublic?: boolean }, identity: Partial<IIdentity<Partial<IClaims>>> | null}) {
...
//         if (queryBe.metadataPerKey.has(arg as keyof queryBe)
// 
...
//           // && queryBe.metadataPerKey.get(arg as keyof queryBe)?.type === typeof args[arg]
//           ) {
...
//     )
    ...
    if (!!args?.HASH && !!args?.RANGE) {
      pk = existingPKWithoutPrefix(queryBe.__typename, { HASH: args.HASH, RANGE: args.RANGE })
    } else {
      pk = newPK(queryBe.__typename, queryBe.__typename)
    }
    this.__typename = queryBe.__typename
    this.HASH = withPrefix(pk.HASH, queryBe.isPublicItem || !!args?.isPublic, identity),
    this.RANGE = withPrefix(pk.RANGE, queryBe.isPublicItem || !!args?.isPublic, identity),
