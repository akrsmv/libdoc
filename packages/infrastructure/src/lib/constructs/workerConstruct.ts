...
export class WorkerConstruct extends Construct {
    ...
    constructor(scope: Construct, id: string, props: CdkStackProps & EventBusConstruct & DynamoDBConstruct) {
        super(scope, id);
        const functionDeadletterQueue = new sqs.Queue(this, "DEADLETTER", {
        ...
        const functionQueue = new sqs.Queue(this, "Queue", {
        ...
        props.eventBus.addSubscription(new sns_subscriptions.SqsSubscription(functionQueue, {
        ...
        this.function = new nodejs.NodejsFunction(this, 'Lambda', {
            ...
            environment: {
                ...
                'QUEUE_URL': functionQueue.queueUrl,
                'EVENT_BUS_TOPIC_ARN': props.eventBus.topicArn
            },
            events: [new lambda_event_sources.SqsEventSource(functionQueue,
                {
        ...
        if (!!props.eventBus) {
            ...
            functionQueue.node.addDependency(props.eventBus)
